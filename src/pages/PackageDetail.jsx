...
export default function PackageDetail() {
  ...
  return (
    <PageWrapper>
      ...
      {/* Main Content */}
      <section className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto" style={{ background: 'var(--surface-page)' }}>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          {/* Left Content */}
          <div className="lg:col-span-2 space-y-10">
            <div>
              <h2 className="font-display text-3xl font-bold mb-4 text-[var(--text-heading)]">Overview</h2>
              <p className="text-lg leading-relaxed text-[var(--text-muted)]">{description}</p>
            </div>

            {/* Destinations covered */}
            <div>
              <h3 className="font-bold text-xl mb-5 text-[var(--text-heading)]">Destinations Covered</h3>
              <div className="flex flex-wrap gap-3">
                {destinations.map((d, i) => (
                  <div key={i} className="flex items-center gap-2 px-4 py-2 glass-light rounded-xl border border-[#1e2d4a]">
                    <MapPin size={14} className="text-emerald-400" />
                    <span className="text-sm font-medium text-[var(--text-muted)]">{d}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Highlights */}
            <div>
              <h3 className="font-bold text-xl mb-5 text-[var(--text-heading)]">Trip Highlights</h3>
            ...
            {/* Includes */}
            <div>
              <h3 className="font-bold text-xl mb-5 text-[var(--text-heading)]">What's Included</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {includes.map((item, i) => (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                <div key={i} className="glass rounded-xl p-4 text-center">
                  ...
                  <div className="text-xs mb-1 text-[var(--text-muted-2)]">{item.label}</div>
                  <div className="text-sm font-semibold text-[var(--text-heading)]">{item.value}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            >
              <div className="text-center mb-6">
                {originalPrice && <div className="text-sm line-through text-[var(--text-muted-2)]">PKR {originalPrice.toLocaleString()}</div>}
                <div className="text-4xl font-bold text-emerald-400">PKR {price.toLocaleString()}</div>
                <div className="text-sm mt-1 text-[var(--text-muted)]">per person • all inclusive</div>
                {discount > 0 && (
              ...
              <div className="space-y-3 mb-6 text-sm">
                <div className="flex justify-between py-2 border-b border-[#1e2d4a]">
                  ...
                  <span className="font-medium text-[var(--text-heading)]">{duration}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-[#1e2d4a]">
                  <span className="text-[var(--text-muted)]">Group Size</span>
                  <span className="font-medium text-[var(--text-heading)]">{groupSize}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-[#1e2d4a]">
                  <span className="text-[var(--text-muted)]">Difficulty</span>
                  <span className="font-medium text-[var(--text-heading)]">{difficulty}</span>
                </div>
                <div className="flex justify-between py-2">
                  <span className="text-[var(--text-muted)]">Rating</span>
                  <span className="font-medium flex items-center gap-1 text-[var(--text-heading)]">
              ...
              <p className="text-center text-xs mt-3 text-[var(--text-muted-2)]">
                {user ? '✓ Ready to book — instant confirmation' : '🔒 Sign in required to complete booking'}
              </p>

              <div className="mt-5 pt-5 border-t border-[#1e2d4a] grid grid-cols-2 gap-3 text-center text-xs text-[var(--text-muted)]">
